...
最终文件放 `docs/media/vision/` 或 `docs/media/events/`。动态功能优先录制连续视频，再从已验收原片无损提取关键帧作为 README 静态图；不要为了同一结论同时重复录制和截图。E01–E03 由 V02 覆盖；E04 使用 V02 的跨区动作并由 V03 的匿名 Track ID 可观测性补强，但不把 Track ID 当作人员身份；E05–E07 由 V03 覆盖，E08–E09 由 V04 覆盖，E10 由 V05 覆盖，E11 由 V12 覆盖；E12–E13 是稳定的完整性/哈希结论，使用单独截图。
...
- 最终重录原片时长 `102.1` 秒，画面 `1280×720`，`30 FPS`，文件约 `7.7 MiB`；清晰度足以用于 GitHub 视频，但不能标注为原生 1080p。
- 连续录到无人、人员进入 Left Zone、持续停留超过 `20` 秒、原路离开并回到无人；开头约 `4` 秒曾短暂落入 Right Zone，发布剪辑从稳定进入 Left Zone 前后开始，避免把无关边界过渡作为核心演示。
- 事件中心录到本轮 Left Zone 的 `ZONE_ENTER`、`ZONE_DWELL`、`ZONE_EXIT`；三条事件详情均显示匿名 Track ID `14`，证据完整性均为 `PASS · 1/1有效 · primary=VALID`，E05–E07 成立。
- Agent 全程使用中文，基于实时查询列出最近事件并正确识别 Track ID `14` 的 `ZONE_ENTER → ZONE_DWELL → ZONE_EXIT` 完整序列，没有将匿名轨迹编号解释为人员身份。
- Harness 显示 `DETERMINISTIC · 1/33`、`COMPLETED · 3 steps`、执行预算 `3/5M · 2/8T`；两个 `event.query` 均为 `L0 · SUCCEEDED`。首次查询由确定性单一只读工具约束强制执行，模型随后主动复查一次，仍在预算内且所有结果一致。
- V03 最终判定通过，不再需要补片或重拍；目录中只保留这份最终 `take01`。README 的 E05–E07 静态图从本视频约 `60–68` 秒的三个事件详情关键帧提取。
- 原片包含音轨，地址栏暴露 LAN IP、“不安全”提示和账户标记；最终发布必须裁切顶栏、隐藏账户并彻底移除音轨。开头画面可见人员面部，必须持续跟踪打码或裁去相应区域。
...
`OBJECT_LEFT_BEHIND` 需要证明“人员与物品同时出现 → 人员离开 → 物品持续存在 → 系统等待确认 → 生成遗留事件”的连续因果关系。单张截图只能证明最终结果，不能证明中间没有剪切、补放物品或伪造事件，所以本项必须录制连续视频；README 的 E10 静态图之后从已验收视频的事件详情帧提取，不再重复拍截图。

**当前实际触发参数**

```text
监控类别：backpack、handbag、suitcase、bottle
稳定库存 minimum_hits：3
物品出现确认：连续 15 帧
遗留确认：连续 200 个无人帧
人员重新布防：连续 15 个有人帧
生产视觉速度：通常约 14–15 FPS
```

理论无人确认时间约 13–14 秒。实拍必须让人离开后等待至少 20 秒，抵抗短时漏检、帧率波动和状态刷新延迟。该规则只确认“无人画面中仍有稳定物品”，不会识别物主或推断是谁遗留了物品。
...
1. 优先录制 `1920×1080`、30 FPS；如果设备只能稳定输出 720p，可录 1280×720，但不得标注为原生 1080p。
2. 录制浏览器窗口，不用手机拍屏。保持缩放 100%，鼠标移动缓慢。
3. 原片可有音轨，但最终发布必须静音；录制过程中不要念密码、IP 或其他秘密。
4. 隐藏书签栏、通知、私人标签页和开发者工具。地址栏与右上角账户可以先保留用于验收，发布剪辑会裁去或遮挡。
5. 使用一个外形标准、已经被当前模型稳定识别为 `bottle` 的瓶子。画面中移走其他瓶、杯、背包或行李箱，避免同时产生多个遗留候选。

**开始前布置**

1. 摄像头固定不动，瓶子最终放在画面中央偏下、光线均匀且不跨区域边界的位置；完整瓶身约占画面高度 25%–45%。
2. 开始时人员和瓶子都在摄像头画面外。Dashboard 顶部应显示 `LIVE`、当前人数 0，稳定物品中没有 bottle。
3. 如果此前已经触发过 bottle 遗留事件，先把瓶子移出画面并等待库存回到 0；本轮录制只认时间与本次动作一致的新事件。
4. 事件中心保持关闭或停在顶部，避免旧事件干扰开场。

**一镜到底动作时间线**

1. **0–5 秒：空场基线。** 开始录屏，停留在 Dashboard 顶部。让实时画面、当前人数 0、稳定物品无 bottle 和 `LIVE` 状态同时可见约 5 秒。
2. **5–12 秒：人携瓶进入。** 测试人员拿着瓶子完整进入画面，面向侧方或让脸处于以后容易打码的位置。进入后至少停留 3 秒，确保连续 15 个有人帧完成重新布防；此时绝不能产生遗留事件。
3. **12–25 秒：放下并建立稳定库存。** 人员把瓶子直立放到预定位置，手离开瓶身，但本人继续站在画面内且不要遮挡瓶子。保持至少 10–12 秒，直到实时框稳定显示 `bottle`、当前人数仍为 1、稳定物品中 bottle 数量为 1。达到后再多等约 3 秒。
4. **25–30 秒：人员离开。** 人员自然离开摄像头画面，瓶子完全不动。离开动作要清楚，不能顺手碰瓶子，也不要在画面边缘徘徊。
5. **30–55 秒：无人确认。** 画面中只剩瓶子，任何人和手都不要再次入画。持续等待至少 20–25 秒，直到当前人数为 0、bottle 仍为 1。不要刷新页面、移动瓶子或中断录制。
6. **55–75 秒：定位新事件。** 缓慢滚动到事件中心，设置：事件类型“物品遗留”、处置状态“待处理”、目标类别 `bottle`、时间范围“最近10分钟”、显示 6 条，然后点击“应用筛选”。
7. **75–95 秒：打开详情。** 打开时间与本轮操作一致的最新“物品遗留 / bottle / Global Scene / MEDIUM”事件。等待证据完整性完成，不要点击“通过 Agent 确认已处理”。
8. **95–110 秒：保留终态。** 在详情中停留 8–12 秒，让标题、北京时间、MEDIUM、bottle、待处理、`PASS · 1/1有效 · primary=VALID` 和主要证据清晰可读。再在详情弹窗内向下滚动到“结构化详情”，只要它显示 `current_count: 1`、`current_people: 0`、`confirmation_frames: 200` 即可结束录制。

**成功验收条件**

- [ ] 全程连续看到空场、人携瓶进入、放下、人员离开、无人等待和事件出现，没有中途剪切。
- [ ] 人员仍在画面时没有产生 `OBJECT_LEFT_BEHIND`。
- [ ] 人离开后 bottle 始终留在原位，当前人数为 0、稳定 bottle 为 1。
- [ ] 新事件类型为“物品遗留”/`OBJECT_LEFT_BEHIND`，目标为 `bottle`，严重级别为 `MEDIUM`，区域为 `Global Scene`。
- [ ] 事件时间与本次离开后的确认时间一致，处置状态仍为“待处理”。
- [ ] 证据完整性为 PASS，主要证据有效且图中是无人但有瓶子的场景。
- [ ] 结构化详情显示 `current_count=1`、`current_people=0`、`confirmation_frames=200`；`current_track_ids` 只是匿名检测轨迹，不代表人员或物主身份。
- [ ] 没有调用写工具、没有确认事件，也没有展示密码、Token、Cookie、私钥或结构化证据路径。

**失败时立即停止，不要继续录**

- 人进入后人数仍为 0：调整站位或光线，重新从空场开始。
- 放瓶 20 秒后稳定库存仍没有 bottle：调整瓶子距离、背景或角度后重录。
- 人还在画面时就出现新遗留事件：不要作为成功素材，记录时间并报告排查。
- 人离开后 bottle 检测或稳定库存变成 0：不要继续等事件，重新摆放并重录。
- 25 秒后仍没有新遗留事件：保持现场不动，先截图当前实时状态并报告，不要编辑数据库或复制旧事件。
- 事件详情不是本轮时间、证据不是 PASS 或 `current_people` 不为 0：不要收片。
