...
- 时长 `148.3` 秒，画面 `960×544`，`30 FPS`，文件约 `3.0 MiB`。分辨率低于计划的 1080p，但原始文字和证据仍可读，可用于网页演示；最终导出可置于 1080p 画布中，不能宣称原生 1080p。
- 连续录到无人、Left Zone `1`、持续停留超过 `20` 秒、人员离开后回到无人；物理动作和区域计数闭环成立。
- 事件中心录到本轮 `ZONE_ENTER`、`ZONE_DWELL`、`ZONE_EXIT`，并打开了进入与离开事件详情；证据图片有效且完整性为 `PASS`。E05–E07 的动态主体成立。
- 事件时间线上还存在一次短暂的额外离开/重新进入，说明边界附近检测或锚点可能发生抖动；公开说明不得把它包装成绝对连续、无丢失的跟踪。
...
- 不重拍物理动作。部署修复后补录一个约 `20–30` 秒的 `take02`：先打开三条历史事件详情显示“匿名 Track ID”，再重新发送同一查询。回答必须按实际记录判断是否一致；若不一致，应明确说明可能存在检测/跟踪中断，并继续禁止推断人员身份。
- 发布剪辑保留约 `0–36` 秒的物理动作、约 `84–116` 秒的事件列表和证据详情，再拼接 `take02`；删除原片约 `120–148` 秒的旧查询回答。
...
H10、D02、D03、D06、D09、D10、D11、V01、V02、V03、V04、V06 和 P01–P04 已完成原片验收。V02 已覆盖 E01–E03 并提供 E04 的跨区动作；V03 的物理动作与区域事件证据已通过，匿名 Track ID 查询补片待录；V04 已同时覆盖 E09 的事件详情与前后证据；发布工程核心证明截图已经齐备。
